import { useWorks } from "@/hooks/useWorks";
import { WorkCard } from "@/components/WorkCard";
import { Skeleton } from "@/components/ui/skeleton";
import { SEOHead } from "@/components/SEOHead";

export default function NovelsPage() {
  const { data: works, isLoading } = useWorks("novel");

  return (
    <main className="container mx-auto px-4 py-10">
      <SEOHead title="Novels" description="Explore original novels and long-form stories by Aakash Agrahari." />
      <h1 className="mb-8 font-serif text-3xl font-bold">Novels</h1>
      {isLoading ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {Array.from({ length: 8 }).map((_, i) => (
            <div key={i} className="space-y-2">
              <Skeleton className="aspect-[3/4] w-full rounded-lg" />
              <Skeleton className="h-4 w-3/4" />
            </div>
          ))}
        </div>
      ) : works && works.length > 0 ? (
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5">
          {works.map((w) => (
            <WorkCard key={w.id} id={w.id} title={w.title} type={w.type} coverImageUrl={w.cover_image_url} wordCount={w.word_count} likeCount={w.like_count} isPinned={w.is_pinned} />
          ))}
        </div>
      ) : (
        <p className="text-muted-foreground">No novels yet.</p>
      )}
    </main>
  );
}
